...
void Queue_Init( Queue *queue ) {
	...
	// Nastavenie všetkých hodnôt v zozname na '*'.
...
int nextIndex( int index ) {
	...
	// Operátorom modula zapezpečíme kruhovosť fronty.
...
int Queue_IsFull( const Queue *queue ) {
	return (nextIndex(queue -> freeIndex) == queue -> firstIndex) || (queue -> freeIndex == 0 && queue -> firstIndex == MAX_QUEUE - 1);
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	...
	// Kontrola prázdnoisti fornty.
	if (Queue_IsEmpty(queue)) {
		Queue_Error(QERR_FRONT);
		return;
	}

	// Priradenie hodnoty do ukazateľa.
...
void Queue_Remove( Queue *queue ) {
	...
	// Poson fronty a vymazanie 0. znaku.
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	...
	// Vloženie hodnoty zo začiatku fronty do cieľového ukazateľa.
	Queue_Front(queue, dataPtr);

	// Odstránenie znaku zo začiatku fronty.
	Queue_Remove(queue);
...
void Queue_Enqueue( Queue *queue, char data ) {
	...
	// Posun indexu na ďalšiu volnú pozíciu.
